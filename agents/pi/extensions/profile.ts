...
function resolveProfileConfigPath(profileName: string, cwd: string): string {
  for (const dir of getAncestorDirs(cwd)) {
    for (const profilesDir of [
      join(dir, ".pi", "profiles"),
      join(dir, ".pi", "agent", "profiles"),
    ]) {
  ...
  throw new Error(
    `Profile "${profileName}" not found in project .pi/profiles, project .pi/agent/profiles, or ${globalProfilesDir}`,
  );
}
...
function parseProfileConfig(
  ...
  if (
    ...
    !VALID_THINKING_LEVELS.has(thinking as ThinkingLevel)
  ) {
    throw new Error(
      `${configPath}: "thinking" must be one of ${Array.from(VALID_THINKING_LEVELS).join(", ")}`,
    );
  }
...
function getDefaultSessionsDir(profileName: string, cwd: string): string {
  ...
  return join(
    ...
    "agent",
    "sessions",
    `${profileName}${safePath}`,
  );
}

function getSessionsDir(profile: ResolvedProfile, cwd: string): string {
  if (profile.sessionsDir && profile.sessionsDir.length > 0)
    return profile.sessionsDir;
...
function loadProfile(profileName: string, cwd: string): ResolvedProfile {
  ...
  return {
    ...
    sessionsDir: config.sessionsDir
      ? resolveFilePath(configDir, config.sessionsDir)
      : undefined,
  };
}
...
export default function profileExtension(pi: ExtensionAPI) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    notifyOrLog(
      ctx,
      `Profile "${profile.name}" does not allow /skill:${requestedSkill}`,
      "error",
    );
    return { action: "handled" as const };
  });
